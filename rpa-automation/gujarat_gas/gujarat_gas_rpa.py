"""
Gujarat Gas RPA Service
Automates name change form filling on Gujarat Gas portal
...
class GujaratGasRPA(BaseRPA):
    def __init__(self, headless=False):
        super().__init__(headless=headless)
        self.portal_url = "https://iconnect.gujaratgas.com/"
        
    def fill_name_change_form(self, form_data):
        """
        Fill Gujarat Gas name change form
        ...
        try:
            self.setup_driver()
            logger.info("=" * 60)
            logger.info("GUJARAT GAS - NAME CHANGE FORM AUTOMATION")
            logger.info("=" * 60)
            ...
            try:
                ...
                for link in links:
                    ...
                    if "service" in text or "request" in text or "name" in text or "change" in text:
            ...
            # Consumer number
            self._fill_field("consumer_number", form_data.get('consumer_number', ''), "Consumer Number")
            ...
            # Take screenshot
            screenshot_path = "/tmp/gujarat_gas_form.png"
