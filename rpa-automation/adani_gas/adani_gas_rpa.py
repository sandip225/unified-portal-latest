"""
Adani Gas RPA Service
Automates name change form filling on Adani Gas portal
...
class AdaniGasRPA(BaseRPA):
    def __init__(self, headless=False):
        super().__init__(headless=headless)
        self.portal_url = "https://www.adanigas.com/"
    ...
    def fill_name_change_form(self, form_data):
        ...
        try:
            self.setup_driver()
            logger.info("=" * 60)
            logger.info("ADANI GAS - NAME CHANGE FORM AUTOMATION")
            logger.info("=" * 60)
            ...
            # Look for name transfer/change link
            logger.info("Looking for name change option...")
            try:
                links = self.driver.find_elements(By.TAG_NAME, "a")
                for link in links:
                    text = link.text.lower()
                    if "name" in text or "transfer" in text or "change" in text:
                        logger.info(f"Found link: {link.text}")
                        link.click()
                        time.sleep(2)
                        break
            except Exception as e:
                logger.warning(f"Could not find name change link: {e}")
            ...
            # Take screenshot
            screenshot_path = "/tmp/adani_gas_form.png"
