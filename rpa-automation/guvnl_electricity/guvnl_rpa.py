"""
GUVNL Electricity RPA Service
Automates name change form filling on GUVNL portal
https://portal.guvnl.in/
Covers: PGVCL, UGVCL, MGVCL, DGVCL
"""
import sys
import os
...
class GUVNLElectricityRPA(BaseRPA):
    def __init__(self, headless=False):
        super().__init__(headless=headless)
        self.portal_url = "https://portal.guvnl.in/login.php"
        
    def fill_name_change_form(self, form_data):
        """
        Fill GUVNL name change form
        ...
        Args:
            form_data: {
                ...
                'distribution': 'PGVCL'  # PGVCL, UGVCL, MGVCL, DGVCL
            }
        """
        try:
            self.setup_driver()
            logger.info("=" * 60)
            logger.info("GUVNL ELECTRICITY - NAME CHANGE FORM AUTOMATION")
            logger.info("=" * 60)
            ...
            # Look for service request or name change
            logger.info("Looking for name change option...")
            try:
                links = self.driver.find_elements(By.TAG_NAME, "a")
                for link in links:
                    text = link.text.lower()
                    if "service" in text or "request" in text or "name" in text:
            ...
            # Take screenshot
            screenshot_path = "/tmp/guvnl_electricity_form.png"
            ...
            return {
                ...
                "portal": "GUVNL",
                "distribution": form_data.get('distribution', 'Unknown'),
                "service": "Name Change"
            }
